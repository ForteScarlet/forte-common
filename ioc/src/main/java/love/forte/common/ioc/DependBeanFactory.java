...
public interface DependBeanFactory {
     ...
     * @return 实例
     */
    <T> T getOrThrow(Class<T> type, Function<NoSuchDependException, NoSuchDependException> exceptionCompute);
     ...
     * @return 转化后的实例
     */
    <T> T getOrThrow(Class<T> type, String name, Function<NoSuchDependException, NoSuchDependException> exceptionCompute);
     ...
     * @return 实例
     */
    Object getOrThrow(String name, Function<NoSuchDependException, NoSuchDependException> exceptionCompute);
    ...
    //  * 根据类型获取所有此类型子类型的依赖。
    //  * <p>
    //  * 很多框架并不支持根据类型获取所有依赖，且有可能存在效率底下的问题，
    //  * 因此请不要过于依赖此方法。
    //  *
